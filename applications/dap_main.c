...
static rt_align(RT_ALIGN_SIZE)
__attribute__((section (".TCM"))) uint8_t cmsisdap_descriptor[] = {
...
chry_ringbuffer_t g_uartrx_for_lvgl;
...
static char mcu_96bit_number_char_data[30] = {0};

// 该函数用于更新序列号
void update_serial_number(uint8_t *descriptor, const char *newSerialNumber)
{
    int serialNumberLen = strlen(newSerialNumber);
    for (int i = 0; i < serialNumberLen; i++)
    {
        descriptor[SERIAL_NUMBER_INDEX + 2 * i] = newSerialNumber[i];
    }
}
...
int chry_dap_init(void)
{
    ...
    chry_ringbuffer_init(&g_uartrx_for_lvgl, uartrx_ringbuffer, CONFIG_UARTRX_RINGBUF_SIZE_FOR_LVGL);
    chry_ringbuffer_init(&g_usbrx, usbrx_ringbuffer, CONFIG_USBRX_RINGBUF_SIZE);

    snprintf(mcu_96bit_number_char_data,sizeof(mcu_96bit_number_char_data),"%d%d%d",(*((uint32_t *)GD32_UID_BASE)),
                            (*((uint32_t *)(GD32_UID_BASE + 4))),
                            (*((uint32_t *)(GD32_UID_BASE + 8))));
    update_serial_number(cmsisdap_descriptor, mcu_96bit_number_char_data);
...
INIT_COMPONENT_EXPORT(chry_dap_init);
...
void usbd_cdc_acm_set_line_coding(uint8_t busid, uint8_t intf, struct cdc_line_coding *line_coding)
{
    if (memcmp(line_coding, (uint8_t *)&g_cdc_lincoding, sizeof(struct cdc_line_coding)) != 0) 
    {
        ...
        config_uart_transfer = 0;
      /* lvgl 显示更新 */
        //set_daplink_uart_is_need_update();
    }
}
    ...
    */
    ...
    if (usbtx_idle_flag) {
        if (chry_ringbuffer_get_used(&g_uartrx)) {
            ...
            usbd_ep_start_write(0, CDC_IN_EP, _usbtx_buffer, size);
        }
    }
